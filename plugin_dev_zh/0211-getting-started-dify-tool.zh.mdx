...
我们的示例需要 `telegraph_access_token`。我们需要在 Provider 配置中定义这个凭证，以便用户在使用插件时可以输入。关于 Provider 配置的更多信息，请参考[通过清单文件定义插件信息](/plugin_dev_zh/0411-plugin-info-by-manifest.zh)。
...
- [高级工具开发](/plugin_dev_zh/9223-tool.zh) - 探索工具插件的高级功能