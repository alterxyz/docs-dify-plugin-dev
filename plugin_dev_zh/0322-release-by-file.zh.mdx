...
> 打包插件前，请确保插件的 `manifest.yaml` 文件和 `/provider` 路径下的 `.yaml` 文件中的 `author` 字段与 GitHub ID 保持一致。关于清单文件的详细信息，请参考[通过清单文件定义插件信息](/plugin_dev_zh/0411-plugin-info-by-manifest.zh)。
